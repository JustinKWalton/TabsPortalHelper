    ...
    static class Installer
    {
        ...
        public static void Install()
        {
            try
            {
                ...
                // Extract the embedded Node + cropper bundle so /process-plan
                // works the moment the helper is installed. Failure here is
                // non-fatal - first /process-plan call retries.
                try
                {
                    if (!CropperBundle.ExtractIfNeeded())
                    {
                        System.Diagnostics.Debug.WriteLine(
                            "CropperBundle extraction failed: " + (CropperBundle.LastError?.Message ?? "unknown"));
                    }
                }
                catch (Exception cex)
                {
                    System.Diagnostics.Debug.WriteLine("CropperBundle threw: " + cex.Message);
                }
